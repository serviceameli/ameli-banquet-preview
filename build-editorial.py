...
# Approved copy changes apply only to the modern page, preserving the original preview.
copy_edits = {
    'problem': [
        ('Устаревший текстиль и мебель', 'Клиентам не из чего выбрать'),
        ('Зал выглядит дешевле, чем стоит на самом деле.',
         'Стандартные мебель и текстиль — сложно предложить оформление под стиль и пожелания конкретного клиента.'),
        ('Закупают устаревшую мебель и декор для мероприятий', 'Нет готовых комплектов оформления'),
        ('Ориентируются на собственный вкус, а не на реальные тренды, которые помогают продавать площадку.',
         'Мебель, текстиль и декор не сочетаются между собой — под каждый запрос приходится искать недостающие элементы и тратить дополнительный бюджет.'),
    ],
    'furniture-options': [
        ('Изготовим мебель под стиль вашей площадки в любом цвете',
         'Изготовим мебель под интерьер вашей площадки'),
        ('Классические модели «Гост», «Кьявари», «Наполеон» и «Кроссбек» — в разных цветах.',
         'Классические модели «Гост», «Кьявари», «Наполеон» и «Кроссбек» и другие в разных цветах.'),
        ('Чёрная модель', 'Сохо'),
        ('Современный чёрный стул', 'Чёрный стул «Сохо»'),
        ('Змейка · овал', 'Составной овал'),
        ('Змейка · круг', 'Столы-конструкторы'),
    ],
    'furniture': [
        ('Все предметы сочетаются между собой по стилю и цветовой гамме.',
         'Подбираем мебель и текстиль, которые сочетаются между собой.'),
    ],
    'reasons': [
        ('Съёмные чехлы и сменные палитры — стиль зала обновляется под запросы сезона без новых затрат.',
         'Обновляйте оформление с помощью сменных чехлов и текстиля.'),
        ('Быстрее закрывают клиентов', 'Клиенту проще выбрать'),
        ('Площадка с готовой концепцией закрывает сделки быстрее — меньше «подумаю», больше «берём».',
         'Покажите готовые варианты оформления и их стоимость — клиенту будет проще сравнить предложения и принять решение.'),
        ('Повторные продажи по рекомендации', 'Новые обращения по рекомендациям'),
        ('Красивый зал → хорошие отзывы и фото → клиенты приходят по рекомендации друзей.',
         'Фотографии мероприятий и отзывы гостей помогают другим клиентам узнать о площадке.'),
        ('Красивый зал позволяет ставить цену выше — клиенты готовы платить за готовую картинку.',
         'Предлагайте мебель, текстиль и сервировку отдельным пакетом в дополнение к аренде зала.'),
    ],
    'business': [
        ('Предложим актуальные стулья, пуфы и столы и сошьём удобные сменные чехлы. Стиль площадки можно будет менять быстро, без больших затрат времени и бюджета и без необходимости выделять дополнительное место для хранения.',
         'Подберём стулья, столы и пуфы. Для моделей со сменными чехлами подготовим несколько цветовых решений — чтобы менять оформление без замены мебели.'),
        ('Поможем обновить ассортимент актуальной посудой, которую легко сочетать с разными палитрами и концепциями оформления.',
         'Подберём тарелки, бокалы и приборы, которые сочетаются с текстилем и подходят для разных мероприятий.'),
    ],
    'visualization': [
        ('Визуализируем заказанный декор прямо в интерьере вашего зала и соберём готовую презентацию для отдела продаж — чтобы менеджер показывал клиенту цельный образ мероприятия, наглядно объяснял ценность оформления и предлагал его как готовое решение.',
         'Покажем выбранную мебель, текстиль и декор в интерьере вашей площадки. Соберём презентацию, с которой менеджерам будет проще предлагать клиентам готовые варианты оформления.'),
    ],
    'textile': [
    ...
    'order': [
        ('Пришлите фотографии или видео зала, план и размеры, вместимость и краткое описание задачи.',
         'Пришлите фотографии зала и расскажите, что хотите обновить. Размеры, вместимость и остальные детали уточним вместе.'),
    ],
}
for section_id, edits in copy_edits.items():
    start = html.index(f'id="{section_id}"')
    end = html.index('</section>', start)
